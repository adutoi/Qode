...
import numpy
import tensorly
from .tensornet import tl_tensor, raw



def _SVDapprox(M, indent, thresh=1e-6):
...
def compress_tensor(nparray_Nd, free_indices, compression="none", _sum_indices=None, _indent="", _first=True):
	...
	if compression=="none":    # means none as in no compression, not as in None being no value given
		...
		if len(free_indices)==1:
			...
			A, B, d = _SVDapprox(nparray_Nd.reshape(tensor_reshape), _indent)
