...
print("relative error in check 12:", check(extract(contract(V(p), F(p,q), V(q))), test))
...
print("relative error in check 13:", check(extract(contract(V(p), evaluate(F)(p,q), V(q))), test))
...
for term in [A, 5*contract(B(0,p), C(p,1))]:    # pretend these would be generated by some algorithm:
    F += term
print("relative error in check 14:", check(extract(contract(V(p), F(p,q), V(q))), test))
...
for term in [A, 5*contract(B(0,p), C(p,1))]:
    increment(F_, term)
